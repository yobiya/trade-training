...
- **データ構造を先に固める**: AI 分析・リアル連携を見越して、シナリオメモ項目・保有中メモ・modeフラグは最初から揃える(タグ・自己評価は不採用、[principles/no-tags.md](./principles/no-tags.md) 参照)。セッション情報・メモテンプレートはファイル管理([§13](./13-data-storage.md) / [§17](./17-data-model.md))、市場データ・経済指標・Setting は SQLite。横断集計はしない([principles/no-aggregation.md](./principles/no-aggregation.md))一方、個別セッションのファイル単位での永続保持と振り返りは許容。
- **market-dataが全ての土台**: データ取得パスがなければ何も始まらない。Phase 1でMT5Provider + キャッシュ層を完成させる
- **アプリ B の MVP は「1 つのセッションを最後まで回せる」最小構成**: チャート表示、エントリー、決済、振り返りメモで決着済みに自動遷移(削除は OS / Dropbox 上の手動操作、[§13](./13-data-storage.md))。横断集計は [principles/no-aggregation.md](./principles/no-aggregation.md) により不採用

...
*仕様書 ver 1.63 - 2026/04/29 (波動ラベルに **ABC 補正波**(エリオット波動の修正波 / フラット・ジグザグ等で使う A-B-C カウント)を追加。波動セレクタに `A` / `B` / `C` ボタンを追加し、`Drawing.data.wave` の値を **文字列で統一**(`'1'|'2'|'3'|'4'|'5'|'A'|'B'|'C'`)。これまでの数値 `1-5` から文字列に切り替えて JSON 上の型を 1 種類に揃え、推進波・補正波を区別なく扱えるようにする(後方互換は不要との方針で、変換層を持たず最も自然な保存形式を選択)。配置 / 移動 / 削除 / TF 別表示などの操作系は推進波と補正波で完全同一。表示は配置時の TF 色で丸 + テキスト中央寄せ。実装スコープは frontend のみで、backend は `data: dict[str, Any]` を素通しするため `schemas/drawing.py` のコメント更新のみ、データモデル / DB 変更なし)*

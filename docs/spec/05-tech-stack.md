...
- **データ構造を先に固める**: AI 分析・リアル連携を見越して、シナリオメモ項目・スタイル・保有中メモ・modeフラグは最初から揃える(タグ・自己評価は不採用、[§7.6](./02-trainer.md#76-タグ構造化選択式入力を採用しない方針) 参照)。ただしセッションデータの長期蓄積はしない方針([§10](./03-analysis.md#10-集計情報蓄積を採用しない方針))

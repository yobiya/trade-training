...
| `takeScreenshot()` | `() => string \| null` | LWC の internal canvas を toDataURL 化 | unmount 時 / 例外時に `null` | AI 分析 PNG 取得用 |
...
| `timeScale.logicalToCoordinate(logical)` | Logical | `number \| null` | **あり** | 範囲外 logical でも px を返す。TF 間 projection はこちらに依存する |
...
### 7.3 「getVisibleTimeRange を Chart 側に持たせる」設計案の却下
